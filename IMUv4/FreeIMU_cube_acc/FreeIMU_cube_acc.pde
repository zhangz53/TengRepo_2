...
void serialEvent(Serial p) {
  if(p.available() >= 34) {
    ...
    if (inputString != null && inputString.length() > 0) {
      ...
      if(inputStringArr.length >= 9) { // q1,q2,q3,q4,\r\n so we have 5 elements
        ...
        tstamp = decodeFloat(inputStringArr[7]);
        
        if((abs(acc[0])<30)&&(abs(acc[1])<30)&&(abs(acc[2])<30)){    //no motion
          vel[0]=0;
          vel[1]=0;
          vel[2]=0;
        }
        
        if((abs(acc[0])<20)&&(abs(acc[1])<20)&&(abs(acc[2])<20)){    //no motion
          pos[0]=0;
          pos[1]=0;
          pos[2]=0;
        }
        
        vel[0]+=(acc[0]/100)*(tstamp/1000);
        vel[1]+=(acc[1]/100)*(tstamp/1000);
        vel[2]+=(acc[2]/100)*(tstamp/1000);
        
        pos[0]+=vel[0]*(tstamp/1000)*100;
        pos[1]+=vel[1]*(tstamp/1000)*100;   
        pos[2]+=vel[2]*(tstamp/1000)*100;
        
        if(pos[0]>50)pos[0]=50;if(pos[0]<-50)pos[0]=-50;
        if(pos[1]>50)pos[1]=50;if(pos[1]<-50)pos[1]=-50;
        if(pos[2]>50)pos[2]=50;if(pos[2]<-50)pos[2]=-50;
...
void drawCube() {  
  pushMatrix();
    translate(VIEW_SIZE_X/2+(pos[1]), VIEW_SIZE_Y/2 + 50-(pos[2]), (pos[0]));
    scale(5,5,5);
