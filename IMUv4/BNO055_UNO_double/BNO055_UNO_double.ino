...
//  q[0]=q2[0]; q[1]=q2[1]; q[2]=q2[2]; q[3]=q2[3];
  ...
  //freeIMUOutRF(&q[0]);
  freeIMUAllThreeOutRF(&q1[0], &q2[0], &q[0]);
}
...
void rawRF_double(){
  ...
  BNO055_4_vec(QUA_DATA_W_LSB, &q1[0]);
  BNO055_4_vec_1(QUA_DATA_W_LSB, &q2[0]);
  freeIMUOutRF_line(&q1[0]);
  RF.write(","); 
  freeIMUOutRF_line(&q2[0]);
  RF.write("\n\r"); 
}
...
byte BNO055_read_1(int addr){
  Wire.beginTransmission(BNO055_I2C_H_ADDR);
  Wire.write(addr);
  Wire.endTransmission(false);

  Wire.requestFrom(BNO055_I2C_H_ADDR, 1, true);
  return Wire.read();
}
...
void freeIMUOut_line(float *quat){
...
void freeIMUOutRF_line(float *quat){
  RF.beginTransmission();
  serialFloatPrintRF(quat[0]);
  RF.write(",");
  serialFloatPrintRF(quat[1]);
...
void freeIMUAllThreeOutRF(float *quat1, float *quat2, float *quat3)
{
  RF.beginTransmission();
  serialFloatPrintRF(quat1[0]);
  RF.write(",");
  serialFloatPrintRF(quat1[1]);
  RF.write(",");
  serialFloatPrintRF(quat1[2]);
  RF.write(",");
  serialFloatPrintRF(quat1[3]);
  RF.write(",");
  serialFloatPrintRF(quat2[0]);
  RF.write(",");
  serialFloatPrintRF(quat2[1]);
  RF.write(",");
  serialFloatPrintRF(quat2[2]);
  RF.write(",");
  serialFloatPrintRF(quat2[3]);
  RF.write(",");
  serialFloatPrintRF(quat3[0]);
  RF.write(",");
  serialFloatPrintRF(quat3[1]);
  RF.write(",");
  serialFloatPrintRF(quat3[2]);
  RF.write(",");
  serialFloatPrintRF(quat3[3]);
  RF.write(",\n");  
  RF.endTransmission();
}
