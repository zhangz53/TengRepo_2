...
void setup()
{
 ...
 Serial.println("BNO055 9-axis motion sensor...");
  byte c = BNO055_read_1(CHIP_ID);
  Serial.println(c, HEX);
  
  Serial.println("check acc_id");
  byte d = BNO055_read_1(ACC_ID);
  Serial.println(d, HEX);  
  delay(1000);
  
  Serial.println("check page_id");
  byte p = BNO055_read_1(PAGE_ID);
...
void freeCubeAllDoubleRF()
{
  ...
  //read acc
  BNO055_3_vec_1(LIA_DATA_X_LSB, &a1[0]);
  
  //read quat
  BNO055_4_vec_1(QUA_DATA_W_LSB, &q1[0]);
  
  ctime=millis();
  t[0]=ctime-ltime;
  ltime=ctime;
  
  freeIMUOut_threeDoubleRF(&a1[0], &q1[0], &t[0]);
...
void freeCubeAllDouble()
{
  ...
  freeIMUOut_threeDouble(&a1[0], &q1[0], &t[0]);
...
void freeIMUOut_threeDoubleRF(float *a, float *q, float *t)
{
  RF.beginTransmission();
  serialFloatPrintRF(a[0]);
  RF.write(",");
  serialFloatPrintRF(a[1]);
  RF.write(",");
  serialFloatPrintRF(a[2]);
  RF.write(",");
...
void freeIMUOut_threeDouble(float *a1, float *q, float *t)
{
