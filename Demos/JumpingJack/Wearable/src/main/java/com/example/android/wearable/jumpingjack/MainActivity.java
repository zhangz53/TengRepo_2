...
public class MainActivity extends Activity
    ...
    private static final long COUNT_DOWN_MS = 1500; // in milliseconds
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //to record
        //renewTimer();
        //storage = DataStorage.getInstance(this, 0);
        //storage.clearData();
        ...
        InputStream rangeStream2 = null;

        try {
            modelStream = assetManager.open("rbf_model_pilot.model");
            rangeStream = assetManager.open("range");
            rangeStream2 = assetManager.open("range2");
        }catch (IOException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }
        predictSVM = new PredictSVM(modelStream, rangeStream, rangeStream2);
    ...
    public void onSensorChanged(SensorEvent event) {
       // detectJump(event.values[0], event.timestamp);
        ...
        //{
          ...
          //  DataStorage.AddSample(sampleCount, event.values[0], event.values[1], event.values[2]);
        //}

        //real-time recognition
        Vector3 tempAcc = new Vector3();
        tempAcc.Set(event.values[0], event.values[1], event.values[2]);
        dataset_acc.add(tempAcc);

        if(dataset_acc.size() > sampleNumber)
        {
    ...
    public void swipeDetect(float x, float y, float z)
    {
    ...
    private void renewTimer() {
        ...
        mTimer = new Timer();
        mTimer.schedule(mTimerTask, COUNT_DOWN_MS, COUNT_DOWN_MS);
    }
